...
function setMedal(s){
  ...
  if (s >= 10) medalEl.className = 'medal bronze';
  if (s >= 20) medalEl.className = 'medal silver';
  if (s >= 30) medalEl.className = 'medal gold';
  if (s >= 40) medalEl.className = 'medal platinum';
}
...
function drawDigit(d, x, y, scale, fill, stroke){
  ...
  // outline (stroke) by drawing slightly larger
...
function drawScoreNumber(numStr, centerX, topY){
  const scale = Math.max(6, Math.floor(W / 70)); // responsive
...
function step(){
  ...
  // score (top center) - even in ready/play/over
  if (state === 'play' || state === 'over'){
    drawScoreNumber(String(score), W/2, Math.max(22, H*0.06));
  }

  // ready hint: show "0" like original vibe
  if (state === 'ready'){
